...
1. Use the flag `--invert_word_order` for the _Inversion_ transformation, `--permute_words` for _Permutation_  and `--one_to_one_mapping` for _Transliteration_. Example command for bilingual pre-training for English with _Inversion_ transformation to create the derived language pair.
``` bash
...
### Evaluation
[This directory](scripts) contains scripts used for evaluation.
...
1. Downstream data - `supervised_data`
1. Model files - `model_outputs`
